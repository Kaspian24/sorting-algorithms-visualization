const gapHibbardFunctionCode = `
function gapHibbard(num: number) {
  let k = 1
...
const gapSedgewickFunctionCode = `
function gapSedgewick(num: number) {
  if (num === 1) {
    return 0
  }
...
const shellSortFunctionCode = (gapFunction1: string, gapFunction2: string) => `
